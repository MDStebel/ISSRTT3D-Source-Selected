...
struct CoordinateCalculations {
    ...
    static func getLongitudeOfSunAtCurrentTime() -> Float {
        ...
        GMT = localHour - secondsFromGMT / Float(Globals.numberOfSecondsInAnHour)
        let noonHourDisplacement = Globals.localNoonTime - GMT - Float(Globals.numberOfHoursInADay)
        let trueLon = (noonHourDisplacement * Globals.degreesLongitudePerHour).truncatingRemainder(dividingBy: 180)
