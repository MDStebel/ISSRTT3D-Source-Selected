...
class AddMarkerForEarthGlobe {
    ...
    /// - Parameters:
    ///   - station: Type of satellite as a SatelliteID
    ...
    init(for station: SatelliteID, using image: String, lat: Float, lon: Float, isInOrbit: Bool) {
        ...
        case .none :
            altitude = Globals.globeRadiusFactor * 0.949                                                                                              // This is the sighting circle, so place it flush with the surface
        }
        
        let position                                     = EarthGlobe.transformLatLonCoordinatesToXYZ(lat: lat, lon: adjustedLon, alt: altitude)      // Map lat and lon to xyz coodinates on globe
